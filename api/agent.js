...
아래 검색 문서에 있는 레시피를 근거로 정확히 3개 추천하세요. 선택한 음식 종류가 있으면 해당 종류를 우선하고, 3개가 부족할 때만 다른 음식 종류를 보충하세요. 검색 문서에 없는 조리법을 새로 지어내지 말고, 재료가 부족하면 missingIngredients에 표시하세요. 제외 메뉴는 반환하지 마세요. description은 카드에 보여 줄 60자 이하의 한 문장 메뉴 소개만 작성하세요. 조리 순서, 시간, 단계, "넣고·볶고·익히고·준비합니다" 같은 과정 설명은 description에 절대 넣지 마세요. 각 메뉴의 steps 배열은 최소 5~6개의 현실적인 조리 단계로 작성하세요. steps의 각 항목에는 한 단계의 동작만 넣고, 여러 단계를 한 문단이나 한 항목에 합치지 마세요. 모든 단계에 구체적인 조리 시간(예: 2~3분), 불 세기(강불/중불/약불), 실제로 그 단계에서 사용하는 재료·조미료와 양만 포함하세요. 사용하지 않는 재료나 조미료는 절대 언급하지 말고, "사용하지 않음", "아직 넣지 않음" 같은 부정 설명도 쓰지 마세요. 음식 종류·필요 재료·태그·조리 시간·난이도는 각각 별도 필드에만 넣고 steps 배열에는 절대 넣지 마세요. 재료 손질, 예열 또는 기름 두르기, 핵심 조리, 간 맞추기, 마무리 순서가 자연스럽게 이어지도록 작성하세요. 반드시 JSON 객체 하나만 반환하세요.
